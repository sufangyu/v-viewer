<template>
  <div>
    ...
    <div v-viewer.rebuild="options" class="images clearfix">
      <template v-for="{source, thumbnail} in images">
        <el-tooltip :key="source" :content="source">
          <el-image :src="thumbnail" :data-source="source" class="image" :key="source" :alt="source.split('?image=').pop()"></el-image>
        </el-tooltip>
      </template>
      <p>{{model}}</p>
...
import { Tooltip, Image, Input } from 'element-ui'
Vue.use(Tooltip)
Vue.use(Image)
Vue.use(Input)
